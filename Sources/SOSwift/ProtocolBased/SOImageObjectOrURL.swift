...
public enum SOImageObjectOrURL: ImageObjectOrURL, Codable {
...
public extension KeyedEncodingContainer {
    mutating func encodeIfPresent(_ value: ImageObjectOrURL?, forKey key: K) throws {
        ...
        if let typedValue = value as? SOImageObjectOrURL {
...
public extension KeyedDecodingContainer {
    func decodeImageObjectOrURLIfPresent(forKey key: K) throws -> ImageObjectOrURL? {
        ...
        do {
            return try self.decodeIfPresent(SOImageObjectOrURL.self, forKey: key)
        } catch {
            return nil
        }
    }
    
    func decodeImageObjectsOrURLsIfPresent(forKey key: K) throws -> [ImageObjectOrURL]? {
        guard self.contains(key) else {
            return nil
        }
