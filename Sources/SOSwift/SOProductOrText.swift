...
public enum SOProductOrText: ProductOrText, Codable {
...
public extension KeyedEncodingContainer {
    mutating func encodeIfPresent(_ value: ProductOrText?, forKey key: K) throws {
        guard let value = value else {
            return
        }
        
        if let typedValue = value as? SOProductOrText {
...
public extension KeyedDecodingContainer {
    func decodeProductOrTextIfPresent(forKey key: K) throws -> ProductOrText? {
        guard self.contains(key) else {
            return nil
        }
        
        do {
            return try self.decodeIfPresent(SOProductOrText.self, forKey: key)
        } catch {
            return nil
        }
    }
    
    func decodeProductsOrTextsIfPresent(forKey key: K) throws -> [ProductOrText]? {
        guard self.contains(key) else {
            return nil
        }
